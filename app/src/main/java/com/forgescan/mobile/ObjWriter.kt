...
// OBJ is inherently multi-file (.obj + .mtl + texture), so it is always
// auto-zipped into a single artifact rather than left as a loose folder.
// Vertex color is still written alongside (some OBJ viewers show it, most
// only honor the material texture) as a fallback for viewers that ignore
// map_Kd.
fun writeObjZip(mesh: ForgeScanMesh, outputZipFile: File, baseName: String = "model") {
    ...
    val objText = buildString {
        ...
        for (v in 0 until vertexCount) {
            // OBJ's texture-coordinate V axis is bottom-up; the atlas (like
            // any raster image) is top-down, so V needs flipping here.
            appendLine("vt ${mesh.uvs[v * 2]} ${1f - mesh.uvs[v * 2 + 1]}")
        }
        ...
        while (i < mesh.indices.size) {
            ...
            appendLine("f $a/$a/$a $b/$b/$b $c/$c/$c")
            i += 3
        }
    }
    val mtlText = buildString {
        ...
        appendLine("map_Kd $baseName.png")
    }
    ...
    ZipOutputStream(outputZipFile.outputStream()).use { zip ->
        ...
        zip.closeEntry()
        zip.putNextEntry(ZipEntry("$baseName.png"))
        zip.write(pngBytesOf(mesh.atlas))
        zip.closeEntry()
    }
}

private fun pngBytesOf(bitmap: Bitmap): ByteArray {
